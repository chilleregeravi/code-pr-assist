...
def test_gpt_summarize_with_context(monkeypatch, provider, expected):
    ...
    if provider == "openai":
        with patch("agent.llm_utils.LLM_PROVIDER", "openai"), \
             patch("openai.ChatCompletion.create") as mock_create:
            mock_create.return_value.choices = [type("obj", (), {"message": type("obj", (), {"content": expected})})]
            result = gpt_summarize_with_context(pr_text, similar_contexts)
            assert result == expected
    else:
        with patch("agent.llm_utils.LLM_PROVIDER", "ollama"), \
             patch("requests.post") as mock_post:
            mock_post.return_value.json.return_value = {"choices": [{"message": {"content": expected}}]}
...
def test_gpt_summarize_with_context_openai_error():
    with patch("agent.llm_utils.LLM_PROVIDER", "openai"), \
         patch("openai.ChatCompletion.create", side_effect=Exception("fail")):
...
def test_gpt_summarize_with_context_unknown_provider():
    with patch("agent.llm_utils.LLM_PROVIDER", "unknown"), \
         patch("openai.ChatCompletion.create") as mock_create:
        result = gpt_summarize_with_context("text", [])
