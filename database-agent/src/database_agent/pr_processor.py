...
import re
from .exceptions import DataValidationError, EmbeddingError, PRProcessingError
...
class PRProcessor:
    ...
    def validate_pr_data(self, pr_data: Dict[str, Any]) -> None:
        ...
        for field, field_type in self.REQUIRED_FIELDS.items():
            ...
            if not isinstance(pr_data[field], field_type):
                raise DataValidationError(
                    f"Invalid type for field {field}: expected {field_type}, got {type(pr_data[field])}"
                )
        ...
        try:
            ...
            if updated_at < created_at:
                raise DataValidationError("Updated date cannot be earlier than created date")
        ...
        if not re.match(r"^[\w.-]+/[\w.-]+$", pr_data["repo_name"]):
            raise DataValidationError(f"Invalid repository name format: {pr_data['repo_name']}")
    ...
    def process_and_store_prs_batch(self, prs_data: List[Dict[str, Any]]) -> None:
        ...
        if errors:
            raise DataValidationError(f"Failed to process some PRs:\n" + "\n".join(errors))
