...
from database_agent.vector_store import QdrantStore, ConfigurationError, ConnectionError, EmbeddingError, VectorStoreError

@pytest.fixture
def store():
    return QdrantStore(url="http://localhost:6333", api_key="test-key", collection_name="test-collection")

def test_initialize_success():
    with patch("database_agent.vector_store.QdrantClient") as mock_client, \
        ...
        store = QdrantStore(url="http://localhost:6333", api_key="test-key", collection_name="test-collection")
        store.initialize()
...
def test_initialize_error():
    with patch("database_agent.vector_store.QdrantClient", side_effect=Exception("fail")):
        store = QdrantStore(url="http://localhost:6333", api_key="test-key", collection_name="test-collection")
