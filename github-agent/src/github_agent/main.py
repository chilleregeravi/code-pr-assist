...
async def handle_pr_webhook(request: Request) -> JSONResponse:
    ...
    with tracer.start_as_current_span("webhook_handler") as span:
        ...
        try:
            ...
            span.set_attribute("pr.number", pr.number)

            full_text = f"Title: {pr.title}\n\n{pr.body}"
            with tracer.start_as_current_span("embedding_agent.embed"):
                embedding = embedding_agent.embed(full_text)
            with tracer.start_as_current_span("embedding_agent.search_similar"):
            ...
            with tracer.start_as_current_span("llm_agent.summarize_with_context"):
            ...
            try:
                with tracer.start_as_current_span("github_agent.post_comment"):
            ...
            try:
                with tracer.start_as_current_span("embedding_agent.upsert"):
