...
def test_embed_text_error(mock_openai):
...
def test_search_similar_success(mock_qdrant):
...
def test_search_similar_error(mock_qdrant):
...
def test_upsert_success(mock_qdrant):
...
def test_upsert_error(mock_qdrant):
